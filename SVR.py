...
X_train=X_train.reshape([2592,9])
y_train=y_train.reshape([2592,1])
X_test=X_test.reshape([360,9])
y_test=y_test.reshape([360,1])

start_time=datetime.now()
# 创建SVR模型并进行训练
svr=SVR(kernel ='rbf',degree = 3,coef0 = 0.0,tol = 0.001,C = 1.0,epsilon = 0.1,shrinking = True,cache_size = 200,verbose = False,max_iter = -1 )
...
print('R2:',R2)
np.save('result/X_test.npy', X_test)
np.save('result/y_test.npy', y_test)
np.savetxt('result/SVR_y_test.txt', y_test, fmt='%.4f')
np.savetxt('result/SVR_y_pred.txt', y_pred, fmt='%.4f')